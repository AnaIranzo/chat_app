...
function socket(io) {
  io.on('connection', (socket) => {
    ...
    socket.on('joined-user', (data) => {
      // Comprobar si hay un usuario y una sala válidos
      if (!data.username || !data.roomname) {
        // Redirigir al usuario a la página de inicio (index.ejs)
        socket.emit('redirect', '/');
        return;
      }

      // Asegurarse de que la sala esté inicializada
    ...
    socket.on('disconnecting', () => {
      ...
      // Verificar si la sala está inicializada antes de acceder a ella
      if (users[roomname]) {
